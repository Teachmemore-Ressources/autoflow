...
#   provision_instance --hostname <name>  (explicit hostname bypasses the
#   k8s-only guard that fires when no hostname is given and
#   AWX_AUTO_DEPROVISION_INSTANCES is False).
# ──────────────────────────────────────────────────────────────────────────────

set -euo pipefail

# Allow non-root images: add dynamic passwd entry when running as UID ≥ 500.
if [ "$(id -u)" -ge 500 ]; then
