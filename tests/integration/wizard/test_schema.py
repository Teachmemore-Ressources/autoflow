...
async def test_version_is_semver(client):
    ...
    assert re.match(r"^\d+\.\d+\.\d+$", body["version"]), f"version {body['version']!r} is not semver"
...
async def test_schema_all_fso_keys_exist_in_fields(client):
    ...
    for fso_key in body["first_start_only"]:
        assert fso_key in field_keys, f"first_start_only key {fso_key!r} has no matching field definition"
...
async def test_schema_known_field_keys_present(client):
    ...
    expected = {"DOMAIN", "POSTGRES_PASSWORD", "REDIS_PASSWORD", "AWX_ADMIN_PASSWORD", "GITEA_ADMIN_PASSWORD"}
