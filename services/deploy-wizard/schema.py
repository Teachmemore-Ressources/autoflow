...
FIELDS = [
    ...
    # ── Gitea LDAP — Connexion ─────────────────────────────────────
    {
        "key": "_GITEA_LDAP_CONN", "label": "Authentification LDAP", "section": "gitea",
        "type": "heading",
        "description": "Optionnel — authentification centralisée via LDAP/Active Directory. Laisser LDAP Mode à « disabled » pour ignorer.",
    },
    {
        "key": "GITEA_LDAP_ENABLED", "label": "Mode LDAP", "section": "gitea",
        "type": "select", "default": "disabled",
        "options": ["disabled", "BindDN", "SimpleAuth"],
        "description": "Type d'authentification LDAP. BindDN = compte de service pour rechercher les utilisateurs ; SimpleAuth = DN construit directement depuis le login",
    },
    {
        "key": "GITEA_LDAP_NAME", "label": "Nom de la source", "section": "gitea",
        "type": "text", "default": "LDAP",
        "description": "Nom affiché pour cette source d'authentification dans l'interface admin Gitea",
    },
    {
        "key": "GITEA_LDAP_HOST", "label": "Hôte LDAP", "section": "gitea",
        "type": "text", "placeholder": "ldap.example.com",
        "description": "Adresse du serveur LDAP ou Active Directory",
    },
    {
        "key": "GITEA_LDAP_PORT", "label": "Port LDAP", "section": "gitea",
        "type": "number", "default": "389",
        "description": "Port du serveur LDAP (389 = non chiffré / StartTLS, 636 = LDAPS)",
    },
    {
        "key": "GITEA_LDAP_SECURITY", "label": "Protocole de sécurité", "section": "gitea",
        "type": "select", "default": "Unencrypted",
        "options": ["Unencrypted", "LDAPS", "StartTLS"],
        "description": "Chiffrement de la connexion LDAP",
    },

    # ── Gitea LDAP — BindDN ────────────────────────────────────────
    {
        "key": "_GITEA_LDAP_BINDDN", "label": "BindDN", "section": "gitea",
        "type": "heading",
        "description": "Compte de service utilisé pour rechercher les utilisateurs dans l'annuaire (mode BindDN uniquement)",
    },
    {
        "key": "GITEA_LDAP_BIND_DN", "label": "Bind DN", "section": "gitea",
        "type": "text", "placeholder": "cn=Search,dc=mydomain,dc=com",
        "description": "DN du compte de service pour la recherche (laisser vide pour recherche anonyme)",
    },
    {
        "key": "GITEA_LDAP_BIND_PASSWORD", "label": "Bind Password", "section": "gitea",
        "type": "password",
        "description": "Mot de passe du compte Bind DN",
    },
    {
        "key": "GITEA_LDAP_USER_SEARCH_BASE", "label": "User Search Base", "section": "gitea",
        "type": "text", "placeholder": "ou=Users,dc=mydomain,dc=com",
        "description": "Base LDAP où rechercher les comptes utilisateurs (requis pour BindDN)",
    },
    {
        "key": "GITEA_LDAP_USER_FILTER", "label": "User Filter", "section": "gitea",
        "type": "text", "wide": True,
        "placeholder": "(&(objectClass=posixAccount)(|(uid=%[1]s)(mail=%[1]s)))",
        "description": "Filtre LDAP pour identifier l'utilisateur — %[1]s est substitué par le login. Pour AD : (&(objectCategory=Person)(sAMAccountName=%s))",
    },
    {
        "key": "GITEA_LDAP_SYNC_USERS", "label": "Synchronisation utilisateurs", "section": "gitea",
        "type": "select", "default": "false",
        "options": ["false", "true"],
        "description": "Synchronisation périodique des utilisateurs Gitea avec l'annuaire LDAP (toutes les 24 h par défaut)",
    },

    # ── Gitea LDAP — Simple Auth ───────────────────────────────────
    {
        "key": "_GITEA_LDAP_SIMPLE", "label": "Simple Auth (alternative à BindDN)", "section": "gitea",
        "type": "heading",
        "description": "Mode Simple Auth — le DN utilisateur est construit directement depuis le login, sans compte de service",
    },
    {
        "key": "GITEA_LDAP_USER_DN", "label": "User DN", "section": "gitea",
        "type": "text", "placeholder": "uid=%s,ou=Users,dc=mydomain,dc=com",
        "description": "Template du DN utilisateur — %s est remplacé par le login. Exemple AD : CN=%s,OU=Users,DC=example,DC=com",
    },

    # ── Gitea LDAP — Attributs ─────────────────────────────────────
    {
        "key": "_GITEA_LDAP_ATTRS", "label": "Mapping des attributs", "section": "gitea",
        "type": "heading",
        "description": "Attributs LDAP à mapper sur le profil Gitea de l'utilisateur",
    },
    {
        "key": "GITEA_LDAP_FIRSTNAME_ATTR", "label": "Attribut prénom", "section": "gitea",
        "type": "text", "default": "givenName",
        "description": "Attribut LDAP pour le prénom",
    },
    {
        "key": "GITEA_LDAP_SURNAME_ATTR", "label": "Attribut nom", "section": "gitea",
        "type": "text", "default": "sn",
        "description": "Attribut LDAP pour le nom de famille",
    },
    {
        "key": "GITEA_LDAP_EMAIL_ATTR", "label": "Attribut email", "section": "gitea",
        "type": "text", "default": "mail",
        "description": "Attribut LDAP pour l'adresse email",
    },
    {
        "key": "GITEA_LDAP_USERNAME_ATTR", "label": "Attribut identifiant", "section": "gitea",
        "type": "text", "placeholder": "uid",
        "description": "Attribut LDAP utilisé comme identifiant Gitea (optionnel — utilise le login saisi par défaut)",
    },
    {
        "key": "GITEA_LDAP_ADMIN_FILTER", "label": "Filtre admin", "section": "gitea",
        "type": "text",
        "description": "Filtre LDAP pour identifier les utilisateurs à promouvoir admin Gitea (optionnel)",
    },

    # ── Gitea LDAP — Groupes ───────────────────────────────────────
    {
        "key": "_GITEA_LDAP_GROUPS", "label": "Vérification des groupes", "section": "gitea",
        "type": "heading",
        "description": "Optionnel — restreindre l'accès Gitea aux membres de certains groupes LDAP",
    },
    {
        "key": "GITEA_LDAP_GROUP_SEARCH_BASE", "label": "Group Search Base DN", "section": "gitea",
        "type": "text", "placeholder": "ou=group,dc=mydomain,dc=com",
        "description": "Base LDAP pour la recherche des groupes",
    },
    {
        "key": "GITEA_LDAP_GROUP_MEMBER_ATTR", "label": "Attribut membre du groupe", "section": "gitea",
        "type": "text", "placeholder": "memberUid",
        "description": "Attribut du groupe listant ses membres — memberUid (LDAP posix) ou member (AD)",
    },
    {
        "key": "GITEA_LDAP_USER_GROUP_ATTR", "label": "Attribut utilisateur référencé", "section": "gitea",
        "type": "text", "placeholder": "uid",
        "description": "Attribut utilisateur référencé dans le groupe — uid si le groupe contient « bender », dn si le groupe contient « uid=bender,ou=… »",
    },
    {
        "key": "GITEA_LDAP_GROUP_FILTER", "label": "Filtre groupes autorisés", "section": "gitea",
        "type": "text", "placeholder": "(|(cn=gitea_users)(cn=admins))",
        "description": "Filtre LDAP pour sélectionner les groupes ayant accès à Gitea (optionnel)",
    },
